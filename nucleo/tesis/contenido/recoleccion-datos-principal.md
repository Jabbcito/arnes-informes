...
3. **Chequeo de sanidad (de conteo, no estadístico)**: comparar el número de filas del CSV contra el `n` registrado en Metodología 3.3. Si difiere bastante, avisar al alumno (puede ser un CSV incompleto, de otro estudio, o de una etapa de piloto por error) — pero no bloquear si el alumno confirma que el número es correcto (por ejemplo, ya descontó la tasa de no-respuesta al calcular `n`, o el diseño es un censo). Los casos que falten respecto al `n` calculado o que tengan valores faltantes se registran en `output/trabajo/reproducibilidad.md` con su motivo — **nunca se eliminan en silencio**: si el CSV llega sin casos o con faltantes, `correlacion.js` reporta los excluidos y eso se explica en Metodología (criterio de exclusión) y en Resultados.
...
5. **Documento de reproducibilidad** (`output/trabajo/reproducibilidad.md`, se crea al terminar la limpieza): registrar las decisiones que afectan el análisis — redondeos aplicados, columnas eliminadas y por qué, casos excluidos con motivo, recodificaciones (ej. inversión de ítems negativos) y el comando exacto que generó cada cifra reportada. Es la definición de **reproducibilidad** de la NASEM (2019): mismos datos + mismo código → mismos resultados. Se distingue de la **replicabilidad** (nuevo muestreo con métodos similares → resultados comparables), que es lo que habilitan los estudios futuros sugeridos en las Limitaciones de la Discusión. Este archivo no se exporta: es evidencia de trabajo para el alumno y material para la sustentación.

...
La auditoría de `auditar-tesis` está en verde (`output/trabajo/checklist-final.md`) — sin ella NO se exporta (regla 16 de `../../../AGENTS.md`).
...
5. Exporta `output/trabajo/informe.md` a DOCX y PDF siguiendo `../../comun/exportacion/exportar-word.md` (Pandoc con `--from markdown+raw_attribute` + filtros `tabla-anchos.lua` e `indice-toc.lua` + `../../comun/exportacion/plantillas/plantilla-apa.docx`) y `../../comun/exportacion/exportar-pdf.md`, confirmando que: el índice de contenidos está presente como campo de Word en su posición correcta (el marcador `::: {#indice}` en `informe.md` tras el Agradecimiento — nunca `--toc` de Pandoc, que lo coloca antes de la carátula) y coincide con el contenido real; las Declaratorias, Dedicatoria, Agradecimiento, Resumen y Abstract quedaron cada uno en su propia página (marcador de salto de página presente en `informe.md`); las Referencias están envueltas en `::: {custom-style="Bibliography"} :::`; **las tablas Markdown se conviertan en tablas nativas de Word editables** con caption nativo (no imágenes ni texto plano); las tablas anchas queden marcadas para orientación horizontal; y la numeración romana/arábiga se arme con los saltos de sección en el acabado final en Word.
...
7. Corre `output/trabajo/checklist-final.md`: rúbrica cubierta, fuentes verificadas, sin evidencia pendiente, matriz de consistencia cuadrada, citas y referencias coherentes, título verificado y coherente, DOCX abre correctamente, PDF revisado.

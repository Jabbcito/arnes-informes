...
4. **Datos principales reales**: Resultados se redactó a partir de `trabajo/datos-principales.csv` (instrumento aplicado a toda la muestra), no del CSV del piloto ni de datos de relleno (regla 26, `../../../tesis/contenido/recoleccion-datos-principal.md`).
...
10. **Título**: sigue describiendo exactamente lo que la tesis terminó siendo; la verificación de disponibilidad está registrada en `trabajo/verificacion-titulo.md`.
11. **Sin `[EVIDENCIA PENDIENTE]`** en el cuerpo (los `[PENDIENTE: firma/Turnitin]` de declaratorias y anexos son los únicos aceptables).
12. Actualiza `trabajo/checklist-final.md` con el resultado de cada punto (✓/✗ + detalle).

## Salida

`trabajo/checklist-final.md` actualizado + resumen para el alumno: qué está en verde, qué falta y quién lo resuelve (arnés / alumno / humano externo).

...
Formato verificado contra la documentación oficial (opencode.ai/docs/skills/): cada skill vive en su propia carpeta como `.opencode/skills/<nombre>/SKILL.md`, con frontmatter `name` (igual al nombre de la carpeta, patrón `^[a-z0-9]+(-[a-z0-9]+)*$`) y `description`. OpenCode las carga bajo demanda vía su tool nativo `skill({ name: "..." })`; en Claude Code/Codex se abren como documento de instrucciones normal (ver `../../CLAUDE.md`).
